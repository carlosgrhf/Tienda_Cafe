...
<head>
	...
	<meta name="Description" content="<?php echo $sis; ?> - <?php echo $fab; ?> - <?php echo $INC_descripcion; ?>" />
        <meta name="author" content="Hay Canal Web S.L." />
	<meta name="Language" content="es" />
	<meta name="Robots" content="index, follow" />
	<title><?php echo $INC_titulo; ?> - <?php echo $sis; ?> - <?php echo $fab; ?></title>
...
<div id="contenedor">   
    ...
    <div class="filtros">
        ...
        $fab_sin = QuitaAcentos($fab);
        //consultamos en la base de datos 
        $sql = "SELECT DISTINCT Pfabricante FROM Productos WHERE Psubfamilia='$sis_sin' AND Pdesactivar='no' ORDER BY Pfabricante asc";
        if(!$result = $mysqli->query($sql)){
            die("Query invalido: " . $sql);
        }
        /* fetch array asociativo*/
        while ($fila = $result->fetch_assoc()) {                
            ...
            echo '<a href="'.$INC_url.'/escaparate_sistema_fabricante/'.$sis_sinysin.'/'.$Pfabricante_arreglado.'" title="'.$Pfabricante.'">'.$Pfabricante.'</a> | ';
        ...
        //consultamos en la base de datos 
        $sql = "SELECT DISTINCT Pfamilia FROM Productos WHERE Psubfamilia='$sis_sin' AND Pdesactivar='no' ORDER BY Pfamilia asc";
        if(!$result = $mysqli->query($sql)){
            die("Query invalido: " . $sql);
        }
        /* fetch array asociativo*/
        while ($fila = $result->fetch_assoc()) {                
            ...
            echo '<a href="'.$INC_url.'/escaparate_sistema_categoria/'.$sis_sinysin.'/'.$Pfamilia_arreglado.'" title="'.$Pfamilia.'">'.$Pfamilia.'</a> | ';
    ...
    //semillas
    echo '<br /><p style="font-size:12pt;">'.$sis.' > '.$fab.'</p>';
