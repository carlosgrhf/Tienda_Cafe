...
//consultamos en la base de datos 
$sql = "SELECT DISTINCT Psubfamilia FROM Productos ORDER BY Psubfamilia asc";
if(!$result = $mysqli->query($sql)){
    die("Query invalido: " . $sql);
}
/* fetch array asociativo*/
...
<head>
	...
	<meta name="Description" content="<?php echo $fab; ?> - <?php echo $sis; ?> - <?php echo $INC_descripcion; ?>" />
        <meta name="author" content="Hay Canal Web S.L." />
	<meta name="Language" content="es" />
	<meta name="Robots" content="index, follow" />
	<title><?php echo $INC_titulo; ?> - <?php echo $fab; ?> - <?php echo $sis; ?></title>
...
<div id="contenedor">   
    ...
    <div class="filtros">
        ...
        $sis_sin = QuitaAcentos($sis);
        $sis_sinysin = QuitaAcentosyEspacios($sis);
        echo '<p>Familia: ';
        //consultamos en la base de datos 
        $sql = "SELECT DISTINCT Pfamilia FROM Productos WHERE Pfabricante='$fab_sin' AND Pdesactivar='no' ORDER BY Pfamilia asc";
        if(!$result = $mysqli->query($sql)){
            die("Query invalido: " . $sql);
        }
        /* fetch array asociativo*/
        while ($fila = $result->fetch_assoc()) {                
            ...
            echo '<a href="'.$INC_url.'/escaparate_fabricante_categoria/'.$fab_sinysin.'/'.$Pfamilia_arreglado.'" title="'.$Pfamilia.'">'.$Pfamilia.'</a> | ';
        ...
        //consultamos en la base de datos 
        $sql = "SELECT DISTINCT Psubfamilia FROM Productos WHERE Pfabricante='$fab_sin' AND Pdesactivar='no' ORDER BY Psubfamilia asc";
        if(!$result = $mysqli->query($sql)){
            die("Query invalido: " . $sql);
        }
        /* fetch array asociativo*/
        while ($fila = $result->fetch_assoc()) {                
            ...
            echo '<a href="'.$INC_url.'/escaparate_fabricante_sistema/'.$fab_sinysin.'/'.$Psubfamilia_arreglado.'" title="'.$Psubfamilia.'">'.$Psubfamilia.'</a> | ';
    ...
    //semillas
    echo '<br /><p style="font-size:12pt;">'.$fab.' > '.$sis.'</p>';
    ...
    //consultamos en la base de datos 
    $sql = "SELECT * FROM Productos WHERE Pfabricante='$fab_sin' AND Psubfamilia='$sis_sin' AND Pdesactivar='no'";
    if(!$result = $mysqli->query($sql)){
        die("Query invalido: " . $sql);
    }
    /* fetch array asociativo*/
